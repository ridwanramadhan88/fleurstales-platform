    ...
    <>
      <div className="storefront-checkout-scroll flex-1 overflow-y-auto px-5 pb-8 pt-5 sm:px-7 sm:pb-10 sm:pt-6 lg:px-8">
        <div className="pb-9 text-center sm:pb-10">
          <span className="mx-auto grid size-16 place-items-center rounded-full bg-[#00813f] text-white sm:size-[4.25rem]"><Check className="size-7 sm:size-8" strokeWidth={1.9} /></span>
          <p className="sf-label mt-5 text-[#00813f]">Order received</p>
          <h3 className="mt-2 text-[2.35rem] font-medium leading-[0.92]">Thank you!</h3>
          <p className="mx-auto mt-3 max-w-sm text-[1.05rem] leading-6 text-black/58">We received your order. The team will confirm it after payment.</p>
        </div>
        ...
        <section className="rounded-[var(--sf-radius-card)] bg-[#f0e6dd] px-5 py-5 sm:px-6 sm:py-6">
          <div className="text-center">
            ...
            <p className="mt-2 text-[2rem] font-medium leading-[0.96]">{placedOrderNumber || 'Pending'}</p>
          </div>
          <div className="mt-5 grid grid-cols-2 gap-5 text-center">
            <div>
              <p className="sf-label text-black/40">{fulfillment === 'delivery' ? 'Delivery' : 'Pickup'} date</p>
            ...
            {paymentAccount ? (
              <div className="space-y-3 px-4 py-4">
                ...
                <div className="flex items-center justify-between gap-3">
                  <p className="text-[1.7rem] font-medium leading-none tabular-nums">{paymentAccount.accountNumber}</p>
                  <StorefrontCopyButton value={paymentAccount.accountNumber} label="Copy" />
                </div>
              </div>
            ) : (
              <p className="px-4 py-3.5 sf-type-2 leading-6 text-black/50">No bank account is currently available for this branch.</p>
            )}
          </div>

          {paymentInstructions.trim() && (
            <details className="mt-3 rounded-[var(--sf-radius-field)] bg-[#f0e6dd] px-4 py-3.5">
              <summary className="cursor-pointer sf-type-2 font-medium text-black/72">Payment note</summary>
      ...
      <footer className="shrink-0 border-t border-black/12 bg-[var(--sf-cream)] px-5 pb-[max(1rem,env(safe-area-inset-bottom))] pt-4 sm:px-7 sm:pb-5 lg:px-8">
        <button type="button" onClick={handleClose} className="sf-primary-action tap-scale w-full px-6 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#00813f]/35 focus-visible:ring-offset-2 focus-visible:ring-offset-[#f0e6dd]">Done</button>
      </footer>
    </>
  )
}
