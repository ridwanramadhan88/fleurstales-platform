...
const POLISHED_TRANSLATION_OVERRIDES: Record<string, string> = {
  ...
  'Period vs Previous Period': 'Periode vs Periode Sebelumnya',
  'Revenue vs Expense': 'Pendapatan vs Pengeluaran',
  'Single view': 'Tampilan tunggal',
...
> = [
  ...
  [/^Revenue \(confirmed\) · (.+)$/i, (_full, period) => `Pendapatan terkonfirmasi · ${period}`],
  [/^Revenue est\. · (.+)$/i, (_full, period) => `Estimasi pendapatan · ${period}`],
  [/^Orders confirmed · (.+)$/i, (_full, period) => `Pesanan terkonfirmasi · ${period}`],
  [/^([^.]+) payroll$/i, (_full, name) => `Penggajian ${name}`],
  [/^Adjust ([A-Z].+)$/i, (_full, name) => `Sesuaikan ${name}`],
  [/^Resolve ([A-Z].+)$/i, (_full, name) => `Selesaikan ${name}`],
  [/^(.+) is now recorded as refunded\.$/i, (_full, order) => `${order} kini tercatat sebagai dana dikembalikan.`],
  [/^(.+) returned to Paid\.$/i, (_full, order) => `${order} kembali ke Lunas.`],
  [/^No customers match "(.+)"\.$/i, (_full, query) => `Tidak ada pelanggan yang cocok dengan "${query}".`],
  [/^Showing (\d+) of (\d+) active orders?\.$/i, (_full, shown, total) => `Menampilkan ${shown} dari ${total} pesanan aktif.`],
  [/^(\d+) decisions? waiting for you\.$/i, (_full, count) => `${count} keputusan menunggu Anda.`],
  [/^Availability for (.+)$/i, (_full, timing) => `Ketersediaan untuk ${timing}`],
  [/^(\d+) recommended · (\d+) active$/i, (_full, recommended, active) => `${recommended} direkomendasikan · ${active} aktif`],
  [/^Approved points and estimated bonus for (.+)\.$/i, (_full, period) => `Poin disetujui dan estimasi bonus untuk ${period}.`],
  [/^Check-out becomes available (\d+) minutes before the end of today's dated shift( at .+)?\.$/i, (_full, minutes, at) => `Check-out tersedia ${minutes} menit sebelum shift bertanggal hari ini berakhir${at ?? ''}.`],
  [/^Your shift ends at (.+)\. Capture a new selfie to check out\.$/i, (_full, time) => `Shift berakhir ${time}. Ambil selfie baru untuk absen pulang.`],
  [/^After payment is confirmed, (.+) becomes available\.$/i, (_full, action) => `Setelah pembayaran dikonfirmasi, ${action} tersedia.`],
  [/^([+-]?\d+) points · (.+)$/i, (_full, points, category) => `${points} poin · ${category}`],
  [/^HR reason: (.+)$/i, (_full, reason) => `Alasan SDM: ${reason}`],
  [/^Value score (\d+)\/100$/i, (_full, score) => `Skor nilai ${score}/100`],
  [/^Check-in ([0-9]{1,2}[:.][0-9]{2})$/i, (_full, time) => `Absen masuk ${time}`],
  [/^Check-out ([0-9]{1,2}[:.][0-9]{2})$/i, (_full, time) => `Absen pulang ${time}`],
  [/^Accepted within (.+) m of this pin\.$/i, (_full, radius) => `Dalam radius ${radius} m dari pin ini.`],
  [/^(\d+) orders$/i, (_full, count) => `${count} pesanan`],
  [/^Rp (.+) lifetime$/i, (_full, amount) => `Total Rp ${amount}`],
  [/^· Last (.+)$/i, (_full, label) => `· Terakhir ${label}`],
  [/^· Score (.+)$/i, (_full, score) => `· Skor ${score}`],
  [/^(.+) · Since (.+)$/i, (_full, contact, date) => `${contact} · Sejak ${date}`],
  [/^(.+) vs yesterday$/i, (_full, delta) => `${delta} vs kemarin.`],
  [/^(\d+) open problems?$/i, (_full, count) => `${count} masalah terbuka`],
  [/^(\d+) completed problems?$/i, (_full, count) => `${count} masalah selesai`],
  [/^(\d+) under review problems?$/i, (_full, count) => `${count} masalah ditinjau`],
  [/^(.+) · Finance confirmed$/i, (_full, period) => `${period} · Dikonfirmasi Keuangan`],
  [/^(.+) · Confirmed \+ (.+) pending$/i, (_full, period, amount) => `${period} · Terkonfirmasi + ${amount} tertunda`],
  [/^(.+) · confirmed orders$/i, (_full, period) => `${period} · pesanan terkonfirmasi`],
  [/^(.+) · per confirmed order$/i, (_full, period) => `${period} · per pesanan terkonfirmasi`],
  [/^(.+) · Finance-confirmed transactions$/i, (_full, period) => `${period} · Transaksi terkonfirmasi Keuangan`],
  [/^(.+) · Confirmed transactions and finished pending orders$/i, (_full, period) => `${period} · Transaksi terkonfirmasi dan pesanan selesai tertunda`],
  [/^Cancel order for (.+)\? This can be undone from the toast immediately after\.$/i, (_full, name) => `Batalkan pesanan ${name}? Ini dapat diurungkan dari toast segera setelahnya.`],
  [/^Remove “(.+)”\?$/i, (_full, name) => `Hapus “${name}”?`],
  [/^No active products use this occasion\.((?: \d+ inactive products? will have this occasion removed\.(?: \d+ whose main occasion is removed will move to Uncategorized\.)?)?)$/i, (_full, tail) => `Tidak ada produk aktif yang memakai momen ini.${tail.replace(/(\d+) inactive products? will have this occasion removed\./, '$1 produk nonaktif ikut kehilangan momen ini.').replace(/(\d+) whose main occasion is removed will move to Uncategorized\./, '$1 di antaranya pindah ke Tanpa kategori.')}`],
  [/^Delete voucher "(.+)"\? This cannot be undone\.$/i, (_full, code) => `Hapus voucher "${code}"? Ini tidak dapat dibatalkan.`],
  [/^Removed occasion “(.+)”\.$/i, (_full, name) => `Momen “${name}” dihapus.`],
  [/^Deleted voucher "(.+)"\.$/i, (_full, code) => `Voucher "${code}" dihapus.`],
  [/^(\d+) unreferenced products? will be deleted\. (\d+) products? used by orders will be archived instead\.$/i, (_full, del, arch) => `${del} produk tak terpakai akan dihapus. ${arch} produk terpakai order akan diarsipkan.`],
  [/^Delete (\d+) products?\? This cannot be undone\.$/i, (_full, count) => `Hapus ${count} produk? Ini tidak dapat dibatalkan.`],
  [/^Delete (\d+) items?\? This cannot be undone\.$/i, (_full, count) => `Hapus ${count} item? Ini tidak dapat dibatalkan.`],
  ...
  // Batch 4: payroll readiness, attendance warnings, payment evidence dynamics.
  [/^[\u00b7\u2022] Showing (\d+) of (\d+)$/i, (_full, shown, total) => `\u00b7 Menampilkan ${shown} dari ${total}`],
  [/^(\d+) employees$/i, (_full, count) => `${count} karyawan`],
  [/^Schedule: (.+)$/i, (_full, value) => `Jadwal: ${value}`],
  [/^Actual: (.+)$/i, (_full, value) => `Aktual: ${value}`],
  [/^Missing check-in \u00b7 (.+)$/i, (_full, rest) => `Belum absen masuk \u00b7 ${polishLegacyCopy(rest)}`],
  [/^Unscheduled attendance \u00b7 (.+)$/i, (_full, rest) => `Kehadiran di hari libur \u00b7 ${polishLegacyCopy(rest)}`],
  [/^(\d+) staff payroll\(s\) will be included after generation\.$/i, (_full, count) => `${count} penggajian staf akan disertakan setelah pembuatan.`],
  [/^(\d+) of (\d+) staff payroll\(s\) included\. Regenerate the proposal\.$/i, (_full, a, b) => `${a} dari ${b} penggajian staf disertakan. Buat ulang proposal.`],
  [/^(\d+) staff payroll\(s\) covered\.?(?: \u00b7 (\d+) manual payee\(s\) added\.)?$/i, (_full, a, b) => (b ? `${a} penggajian staf tercakup \u00b7 ${b} penerima manual ditambahkan.` : `${a} penggajian staf tercakup.`)],
  [/^(\d+) employee\(s\) need an? base salary\.$/i, (_full, count) => `${count} karyawan membutuhkan gaji pokok.`],
  [/^(\d+) attendance warning\(s\) must be confirmed or corrected before payroll submission\.$/i, (_full, count) => `${count} peringatan kehadiran harus dikonfirmasi atau dikoreksi sebelum pengajuan penggajian.`],
  [/^(\d+) manual point adjustment\(s\) are pending and excluded until approved\.$/i, (_full, count) => `${count} penyesuaian poin manual menunggu dan dikecualikan sampai disetujui.`],
  [/^(\d+) payroll calculation\(s\) need correction\.$/i, (_full, count) => `${count} perhitungan penggajian perlu dikoreksi.`],
  [/^Checked in (\d+) minutes? after the allowed grace period\.$/i, (_full, count) => `Absen masuk ${count} menit setelah batas toleransi.`],
  [/^Checked out (\d+) minutes? after the scheduled end time\.$/i, (_full, count) => `Absen pulang ${count} menit setelah jam selesai yang dijadwalkan.`],
  [/^No checkout recorded (\d+) minutes? after the checkout grace period\.$/i, (_full, count) => `Absen pulang belum tercatat ${count} menit setelah toleransi absen pulang.`],
  [/^Detected at (.+) instead of scheduled branch (.+)\.$/i, (_full, a, b) => `Terdeteksi di ${a}, bukan di cabang terjadwal ${b}.`],
  [/^Private Finance evidence \u00b7 max (\d+) KB after compression\.$/i, (_full, kb) => `Bukti privat Keuangan \u00b7 maks. ${kb} KB setelah kompresi.`],
  [/^Legacy ledger status: (.+)\. No Finance approval action is required in this workflow\.$/i, (_full, status) => `Status ledger lama: ${status}. Tidak perlu persetujuan Keuangan dalam alur ini.`],
  [/^(.+) is PAID\. Process Order is now available to assign a florist\.$/i, (_full, order) => `${order} LUNAS. Proses Pesanan kini tersedia untuk menugaskan florist.`],
]
