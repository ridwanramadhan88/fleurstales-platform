  ...
  return <PeoplePageShell>
    ...
    >
      ...
      {canCreateAccounts ? <section className="space-y-5 rounded-xl bg-card p-4 ring-1 ring-border/70">
        <div className="space-y-1"><h3 className="text-sm font-semibold leading-5">Login access</h3><p className="text-sm leading-5 text-muted-foreground">{usesSupabaseStaffAuth ? 'Send a secure Supabase invitation to the staff email.' : 'Create the local staff username and six-digit PIN.'}</p></div>
        {usesSupabaseStaffAuth ? <Field label="Staff email" required error={formErrors.email}><input required type="email" value={form.email} onChange={vm.onFormFieldChange('email')} className={createStaffFieldInputClass(formErrors.email)} placeholder="staff@fleurstales.com" /></Field> : <div className="grid gap-5 sm:grid-cols-2"><Field label="Username" required error={formErrors.username}><input required value={form.username} onChange={vm.onFormFieldChange('username')} className={createStaffFieldInputClass(formErrors.username)} placeholder="lowercase" /></Field><Field label="PIN" required error={formErrors.pin}><input required inputMode="numeric" maxLength={6} value={form.pin} onChange={vm.onFormFieldChange('pin')} className={createStaffFieldInputClass(formErrors.pin)} placeholder="6 numbers" /></Field></div>}
      ...
      {canCreateAccounts && <section className="rounded-xl bg-surface-panel p-4 ring-1 ring-border/70"><h3 className="text-sm font-semibold leading-5">Account setup check</h3><div className="mt-3 grid gap-2 text-sm sm:grid-cols-2"><span className={form.name.trim() ? 'text-success' : 'text-muted-foreground'}>{form.name.trim() ? '✓' : '○'} Name added</span><span className={assignableRoles.includes(form.systemRole) ? 'text-success' : 'text-muted-foreground'}>{assignableRoles.includes(form.systemRole) ? '✓' : '○'} Allowed role selected</span>{usesSupabaseStaffAuth ? <span className={/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email) ? 'text-success' : 'text-muted-foreground'}>{/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email) ? '✓' : '○'} Staff email ready</span> : <><span className={/^[a-z0-9._-]+$/.test(form.username) ? 'text-success' : 'text-muted-foreground'}>{/^[a-z0-9._-]+$/.test(form.username) ? '✓' : '○'} Username ready</span><span className={/^\d{6}$/.test(form.pin) ? 'text-success' : 'text-muted-foreground'}>{/^\d{6}$/.test(form.pin) ? '✓' : '○'} Six-digit PIN ready</span></>}<span className={form.hireDate ? 'text-success' : 'text-muted-foreground'}>{form.hireDate ? '✓' : '○'} Start date added</span></div></section>}
        ...
        {detailsEmployee && detailsForm && <div className="max-h-[70dvh] space-y-4 overflow-y-auto px-5 py-4">
          ...
          <section className={`${detailsSection==='access'?'block':'hidden'} rounded-xl bg-muted/35 p-4 ring-1 ring-border/60`}>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              {usesSupabaseStaffAuth ? <Field label="Staff email"><input disabled value={detailsEmployee.email ?? 'Managed in Supabase Auth'} className={fieldInputClass()} /></Field> : <><Field label="Username" error={accessErrors.username}><input disabled={!canEditCredentials} value={detailsForm.username} onChange={(e) => vm.onDetailsFieldChange('username', e.target.value.toLowerCase())} className={fieldInputClass(accessErrors.username)} /></Field><Field label="New PIN" error={accessErrors.pin}><input disabled={!canEditCredentials} inputMode="numeric" maxLength={6} value={detailsForm.pin} onChange={(e) => vm.onDetailsFieldChange('pin', e.target.value.replace(/\D/g, '').slice(0, 6))} placeholder="Leave blank to keep current PIN" className={fieldInputClass(accessErrors.pin)} /></Field></>}
            </div>
