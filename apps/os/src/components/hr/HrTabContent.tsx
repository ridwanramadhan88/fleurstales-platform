  ...
  return <PeoplePageShell>
    ...
    >
      ...
      {canCreateAccounts ? <section className="space-y-5 rounded-xl bg-card p-4 ring-1 ring-border/70">
        <div className="space-y-1"><h3 className="text-sm font-semibold leading-5">Login access</h3><p className="text-sm leading-5 text-muted-foreground">Email is used for secure password or PIN recovery. Staff can sign in with their username or email.</p></div>
      ...
      {canCreateAccounts && <section className="rounded-xl bg-surface-panel p-4 ring-1 ring-border/70"><h3 className="text-sm font-semibold leading-5">Account setup check</h3><div className="mt-3 grid gap-2 text-sm sm:grid-cols-2"><span className={form.name.trim() ? 'text-success' : 'text-muted-foreground'}>{form.name.trim() ? '✓' : '○'} Name added</span><span className={assignableRoles.includes(form.systemRole) ? 'text-success' : 'text-muted-foreground'}>{assignableRoles.includes(form.systemRole) ? '✓' : '○'} Allowed role selected</span><span className={/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email) ? 'text-success' : 'text-muted-foreground'}>{/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email) ? '✓' : '○'} Recovery email ready</span><span className={/^[a-z][a-z0-9._-]*$/.test(form.username) ? 'text-success' : 'text-muted-foreground'}>{/^[a-z][a-z0-9._-]*$/.test(form.username) ? '✓' : '○'} Username ready</span><span className={/^\d{6}$/.test(form.pin) ? 'text-success' : 'text-muted-foreground'}>{/^\d{6}$/.test(form.pin) ? '✓' : '○'} Six-digit PIN ready</span><span className={form.hireDate ? 'text-success' : 'text-muted-foreground'}>{form.hireDate ? '✓' : '○'} Start date added</span></div></section>}
        ...
        {detailsEmployee && detailsForm && <div className="max-h-[70dvh] space-y-4 overflow-y-auto px-5 py-4">
          ...
          <section className={`${detailsSection==='access'?'block':'hidden'} rounded-xl bg-muted/35 p-4 ring-1 ring-border/60`}>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <><Field label="Username" error={accessErrors.username}><input disabled={!canEditCredentials} value={detailsForm.username} onChange={(e) => vm.onDetailsFieldChange('username', e.target.value.toLowerCase())} className={fieldInputClass(accessErrors.username)} /></Field><Field label="New password / PIN" error={accessErrors.pin}><input disabled={!canEditCredentials} inputMode="numeric" maxLength={6} value={detailsForm.pin} onChange={(e) => vm.onDetailsFieldChange('pin', e.target.value.replace(/\D/g, '').slice(0, 6))} placeholder="Leave blank to keep current credential" className={fieldInputClass(accessErrors.pin)} /></Field></>
            </div>
