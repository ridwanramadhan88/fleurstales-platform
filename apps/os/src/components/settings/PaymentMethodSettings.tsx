...
export const PaymentMethodSettings: FC<Props> = ({
  ...
  <section className="space-y-8">
    <div className="space-y-5">
      <SettingsSectionHeader icon={CreditCard} title="Payment accounts" description="Accounts used for customer payments and Finance money movement, plus checkout instructions." />
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between gap-3">
          ...
          {isEditing && <button type="button" onClick={onAddBankAccount} className="inline-flex items-center bg-primary text-sm font-semibold text-primary-foreground shadow-sm rounded-full px-[18px] whitespace-nowrap h-11 gap-2"><Plus className="size-3.5" /> Add account</button>}
        </div>
    ...
    {isEditing && (
      <p className="rounded-xl bg-info/10 px-4 py-3 text-xs text-info ring-1 ring-info/20">
        Save payment-account changes first. Transaction categories are managed from this Finance section after leaving edit mode.
      </p>
    )}
  </section>
)
