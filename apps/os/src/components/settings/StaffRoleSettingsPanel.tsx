            ...
            {staffAccountDraft && (
              <div className="mt-4 rounded-xl bg-surface-panel p-3 ring-1 ring-border/60">
                ...
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                  ...
                  {usesSupabaseStaffAuth ? (
                    <Field label="Staff email" error={validationErrors['staff.account.email']}><input type="email" value={staffAccountDraft.email} onChange={(event) => onUpdateStaffAccountDraft({ email: event.target.value.toLowerCase() })} placeholder="staff@fleurstales.com" className={FIELD_CLASS} /></Field>
                  ) : (
                    <>
