 ...
 * - Splits customer management and submitted review history into dedicated
 *   sub-tabs so voucher and review-promo actions stay with their own workspace.
 *
...
type CustomerWorkspaceTab = 'customers' | 'reviews'
...
export const CustomersTabContent: FC<CustomersTabContentViewModel> = ({
  ...
  return (
    <section className="space-y-4">
      ...
      <nav aria-label="Customer workspace" className="flex w-full gap-1 rounded-xl bg-muted/55 p-1 sm:w-fit">
        <button
          type="button"
          aria-current={workspaceTab === 'customers' ? 'page' : undefined}
          ...
          className={`min-h-10 flex-1 rounded-lg px-4 text-sm font-semibold transition sm:flex-none ${
            workspaceTab === 'customers'
              ? 'bg-card text-foreground shadow-ios-sm ring-1 ring-border/60'
        ...
        <button
          type="button"
          aria-current={workspaceTab === 'reviews' ? 'page' : undefined}
