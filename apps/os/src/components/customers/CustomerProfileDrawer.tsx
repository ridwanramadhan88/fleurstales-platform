...
/** Tone + label per order status, matching status chips elsewhere. */
    ...
    >
      <header className="mb-3 flex items-start justify-between gap-3">
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0 space-y-1.5">
            <p className="text-2xs font-semibold text-muted-foreground">Customer</p>
            <div className="flex flex-wrap items-center gap-2">
              <h2 className="truncate text-lg font-semibold leading-6 text-foreground">{customer.name}</h2>
            ...
            <p className="text-xs text-muted-foreground">Value score {valueScore}/100</p>
          </div>
        </div>
      </header>
      ...
      <div className="min-h-0 flex-1 space-y-3 overflow-y-auto overflow-x-hidden px-1 pb-4 pt-1 md:grid md:grid-cols-[minmax(18rem,0.9fr)_minmax(24rem,1.1fr)] md:items-start md:gap-4 md:space-y-0">
        <div className="space-y-3">
          <section className="grid grid-cols-2 gap-3 rounded-xl bg-card px-4 py-4 ring-1 ring-border/70">
            <div className="flex items-start gap-2">
              <span className="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-muted text-muted-foreground"><Wallet className="size-3.5" /></span>
              <div className="min-w-0">
                <p className="text-2xs font-semibold text-muted-foreground">Lifetime spend</p>
                <p className="text-sm font-semibold leading-5 text-foreground">Rp {formatter.format(metrics.lifetimeSpend)}</p>
              </div>
            </div>
            <div className="flex items-start gap-2">
              <span className="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-muted text-muted-foreground"><ShoppingBag className="size-3.5" /></span>
              <div className="min-w-0">
                <p className="text-2xs font-semibold text-muted-foreground">Orders</p>
                <p className="text-sm font-semibold leading-5 text-foreground">{metrics.orderCount}</p>
              </div>
            </div>
            <div className="flex items-start gap-2">
              <span className="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-muted text-muted-foreground"><TrendingUp className="size-3.5" /></span>
              <div className="min-w-0">
                <p className="text-2xs font-semibold text-muted-foreground">Avg order value</p>
                <p className={metrics.averageOrderValue != null ? 'text-sm font-semibold text-foreground' : 'text-sm text-muted-foreground/45'}>
                  {metrics.averageOrderValue != null ? `Rp ${formatter.format(metrics.averageOrderValue)}` : 'Not available'}
                </p>
              </div>
            </div>
            <div className="flex items-start gap-2">
              <span className="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-muted text-muted-foreground"><MapPin className="size-3.5" /></span>
              <div className="min-w-0">
                <p className="text-2xs font-semibold text-muted-foreground">Preferred branch</p>
          ...
          <section className={`space-y-1.5 rounded-lg px-3 py-2.5 ${customer.notes || (customer.tags && customer.tags.length > 0) ? 'bg-muted' : 'bg-muted/20 ring-1 ring-border/30'}`}>
            <p className={`text-2xs font-semibold ${customer.notes || (customer.tags && customer.tags.length > 0) ? 'text-muted-foreground' : 'text-muted-foreground/40'}`}>Notes & tags</p>
            <div className="space-y-1.5">
              {customer.tags && customer.tags.length > 0 && (
                <div className="flex flex-wrap gap-1">
                  {customer.tags.map((tag) => (
                    <span key={tag} className="inline-flex items-center rounded-full bg-card px-2.5 py-1 text-2xs font-medium text-foreground/90 ring-1 ring-border/70">{tag}</span>
                  ))}
                </div>
              )}
              <p className={customer.notes ? 'text-sm text-foreground/90' : 'text-xs text-muted-foreground/45'}>{customer.notes ?? 'Not added'}</p>
            </div>
          </section>
        ...
        <section className={`space-y-2 rounded-xl px-4 py-3.5 ${orders.length > 0 ? 'bg-muted' : 'bg-muted/20 ring-1 ring-border/30'}`}>
          <p className={`text-2xs font-semibold ${orders.length > 0 ? 'text-muted-foreground' : 'text-muted-foreground/40'}`}>Order history</p>
          {orders.length === 0 ? (
            <p className="text-xs text-muted-foreground/45">No linked orders</p>
          ) : (
            <ul className="space-y-1.5">
              {orders.map((order) => {
                const statusMeta = ORDER_STATUS_META[order.status]
                return (
                  <li key={order.orderNumber} className="flex items-center justify-between gap-2 rounded-lg bg-card px-3 py-2 ring-1 ring-border/70">
                    <div className="min-w-0">
                      <p className="truncate text-sm font-medium text-foreground">{order.orderNumber}</p>
                      <p className="truncate text-xs text-muted-foreground">{order.createdAtLabel} · {order.branch}</p>
                    </div>
                    <div className="flex shrink-0 flex-col items-end gap-1">
                      <p className="text-sm font-semibold leading-5 text-foreground">Rp {formatter.format(order.totalIdr)}</p>
                      <StatusChip tone={statusMeta.tone} className="px-2 py-0.5 text-2xs">{statusMeta.label}</StatusChip>
                    </div>
                  </li>
                )
              })}
