...
export const OrderDetailsPanel: FC<OrderDetailsViewModel> = (viewModel) => {
  ...
  return (
    ...
    <AppSheet
      ...
      contentClassName="max-w-6xl gap-0 overflow-hidden rounded-t-3xl bg-card px-5 pb-4 pt-5 shadow-2xl ring-1 ring-border/35 sm:right-auto sm:h-[92vh] sm:max-h-[92vh] sm:w-[calc(100vw-2rem)] sm:px-6 sm:pb-5 sm:pt-5 lg:h-[90vh] lg:max-h-[90vh]"
    >
      <OrderDetailsHeader viewModel={viewModel} />
      <OrderDetailsFinanceSection viewModel={viewModel} />

      <div className="mt-4 min-h-0 flex-1 overflow-y-auto overflow-x-hidden pb-10 pt-1 text-sm text-foreground/90">
        <div className="space-y-8 sm:grid sm:grid-cols-5 sm:items-start sm:gap-8 sm:space-y-0">
