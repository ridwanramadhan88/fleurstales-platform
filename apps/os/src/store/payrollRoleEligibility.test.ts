...
describe('payroll role eligibility and independent Finance review', () => {
  ...
  it('allows Finance group approval and records matching-account self-approval', () => {
    const hr = { name:'Bintang', role:'hr' as const }
    ...
    expect(usePayrollStore.getState().payrollProposalReviews.at(-1)).toMatchObject({
      decision:'approved',
      selfApprovalEmployeeIds:['emp-finance'],
      actorName:'Dewi',
    })
  })
})
