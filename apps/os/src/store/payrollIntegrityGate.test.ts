...
describe('payroll readiness and schedule locking',()=>{
 ...
 it('blocks submission until attendance warnings are resolved',()=>{
  const result=usePayrollStore.getState().submitPayrollToFinance({payrollPeriodId:period.id,actor:{name:'HR',role:'hr'}})
  expect(result).toMatchObject({ok:false,code:'pending_attendance'})
  expect(usePayrollStore.getState().payrollProposals).toHaveLength(0)
 })
