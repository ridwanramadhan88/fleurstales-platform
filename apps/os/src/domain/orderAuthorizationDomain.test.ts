...
describe('order row-level authorization', () => {
  ...
  it('limits branch-scoped Admin access while keeping Owner and Finance cross-branch', () => {
    expect(canViewOrder(kedamaianOrder,{ employeeId:'admin-b',name:'Admin B',role:'admin',branchId:'Pahoman' },DEFAULT_ROLE_SECTION_ACCESS)).toBe(false)
  ...
  it('denies branch-scoped staff when no active branch assignment exists', () => {
    expect(canViewOrder(kedamaianOrder,{ employeeId:'admin-off-shift',name:'Admin Off Shift',role:'admin' },DEFAULT_ROLE_SECTION_ACCESS)).toBe(false)
    expect(authorizeOrderMutation({ order:kedamaianOrder, actor:{ employeeId:'admin-off-shift',name:'Admin Off Shift',role:'admin' }, permissions:DEFAULT_ROLE_SECTION_ACCESS, kind:'status' })).toMatchObject({ allowed:false })
  })

  it('limits Florists to orders assigned to their employee identity', () => {
  ...
  it('denies direct mutations outside the actor row scope', () => {
    expect(authorizeOrderMutation({ order:kedamaianOrder, actor:{ employeeId:'admin-b',name:'Admin B',role:'admin',branchId:'Pahoman' }, permissions:DEFAULT_ROLE_SECTION_ACCESS, kind:'details' })).toMatchObject({ allowed:false })
  })
})
