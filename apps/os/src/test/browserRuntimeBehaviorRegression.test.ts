...
describe('browser runtime behavior', () => {
  ...
  it('restores sessions behind a loading surface and resets workspace navigation scroll', () => {
    ...
    expect(login).toContain('Memulihkan sesi aman Anda')
    expect(login).toContain('userFacingLoginError')
    expect(login).toContain("message.includes('SESSION_REQUIRED')")
    expect(login).toContain('Sesi aman Anda telah berakhir. Silakan masuk kembali.')
    expect(login).toContain('Gunakan nama pengguna atau email staf dan kata sandi.')
    expect(login).toContain('Lupa kata sandi?')
  ...
  it('binds an open production workspace to the same authoritative Supabase user', () => {
    ...
    expect(app).toContain("const productionSession = sharedSession.source === 'supabase'")
    expect(app).toContain('const authSession = await initializeSupabaseAuth()')
    expect(app).toContain('authSession.user.id !== expectedUserId')
    expect(app.indexOf('const authSession = await initializeSupabaseAuth()')).toBeLessThan(app.indexOf("runHydrationStage('Authorization'"))
    expect(app).toContain("runHydrationStage('Operational domains'")
    expect(app).toContain('getAuthorizationHydrationError')
    expect(app).toContain('getInternalSettingsHydrationError')
    expect(app).toContain('usePersistenceHealthStore.getState().message')
  ...
  it('keeps workflow descriptions contextual and resolves line-item product photos', () => {
    ...
    const financeDetails = read('src/components/finance/OrderFinanceReviewSheetDetails.tsx')
    const financeHeader = read('src/components/finance/OrderFinanceReviewSheetHeader.tsx')
    const financeSummary = read('src/components/finance/OrderFinanceReviewProductSummary.tsx')
    ...
    expect(orderItems).not.toContain('Current status')
    expect(orderItems).not.toContain('itemCount')
    expect(financeDetails).not.toContain('STATUS_LABELS')
    expect(financeHeader).toContain('formatOrderCreatedAtLabel')
    expect(financeHeader).not.toContain('Order · view only')
    expect(financeSummary).not.toContain('itemCount')
  })
})
