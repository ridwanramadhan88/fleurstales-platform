...
describe('chip, tab, card and section spacing system', () => {
  ...
  it('defines only the three intended card densities and shared internal rhythm', () => {
    ...
    expect(card).toContain("dense: 'p-4'")
    expect(card).toContain("standard: 'p-5'")
    expect(card).toContain("summary: 'p-6'")
    expect(card).toContain("'rounded-2xl bg-surface-card ring-1 ring-border/60'")
    expect(card).toContain("options?.shadow === false ? undefined : 'shadow-ios-sm'")
    expect(card).toContain("cardHeaderClass = 'flex items-start justify-between gap-3'")
    expect(card).toContain("cardBodyClass = 'mt-3'")
    expect(card).toContain("cardActionsClass = 'mt-4'")
    expect(card).toContain("dividedCardActionsClass = 'mt-4 border-t border-border/60 pt-4'")
    expect(overviewCard).toContain('min-h-20')
    expect(overviewCard).toContain("surfaceCardClass('dense'")
  })
  ...
  it('uses parent-driven section spacing and consistent focused-workflow clearance', () => {
    ...
    expect(financeReview).toContain('space-y-8')
